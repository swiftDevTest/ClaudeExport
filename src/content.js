(function initChatVaultContentScript() {
  ...
  const EXPORT_SETTINGS_STORAGE_KEY = _storageKey("export_settings.v1");
  ...
  async function syncVerifiedExportEntitlement(conversationCount, options = {}) {
    ...
    try {
      ...
      // 网络/未知错误：降级为本地配额，避免后端短暂故障导致用户完全无法导出
      const localAccess = getLocalExportAccessResult(count);
      console.warn("Session refresh failed; using local quota fallback:", error);
      return localAccess;
    }

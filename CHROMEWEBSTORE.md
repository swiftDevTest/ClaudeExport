...
5. **用户 activity / 网络历史 (User activity / Web history)**:
   - *是否收集*: 否
   - *是否传输*: 否
   - *说明*: 扩展仅限在 Claude 域名下运作。不追踪也不记录用户的浏览历史或搜索行为。
...
| `1.3.0` | 2026-07-27 | Notion 同步产品隔离与安全边界强化；修复 Claude 思考块元数据误并入正文导出问题；欢迎页与弹窗 UI 重构；订阅流程与会话持久化稳定性提升；移除遗留 redaction/offscreen 模块。 | 待提交 (Draft) |
